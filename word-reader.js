...
export class WordReader {
    ...
    getKnownWords() {
        ...
        Object.values(this.dictionary).forEach(entries => {
            entries.forEach(entry => {
                if (entry && entry.base_word) {
                    knownWords.add(entry.base_word.toLowerCase());
                }
            });
        });
    ...
    processWords(content) {
		...
		try {
			...
			const contentWithoutPhrases = content.replace(/<span class="known-phrase"[^>]*>(.*?)<\/span>/g, (match) => {
				const id = phraseElements.length;
				phraseElements.push(match);
				return `###PHRASE${id}###`;
			});
	
			// 2. Apdorojame likusį tekstą, naudojant tik žodžius su tarpais
			const wordPattern = /[A-Za-zåäöÅÄÖ]+/g;
			let result = contentWithoutPhrases.replace(wordPattern, (word) => {
				const wordLower = word.toLowerCase();
				if (this.dictionary[wordLower]) {
					return `<span class="known-word" data-word="${word}">${word}</span>`;
				}
				return word;
			});
			...
			phraseElements.forEach((phrase, index) => {
				result = result.replace(`###PHRASE${index}###`, phrase);
			});
        ...
        try {
            ...
            knownWords.forEach(wordElement => {
                const handleWordClick = (e) => {
                    ...
                    if (!word) {
                        console.warn('Nerasta žodžio reikšmė elemente:', e.target);
                        return;
                    }
                ...
                // Pridedame click event listener
                wordElement.addEventListener('click', handleWordClick);

                // Pridedame touch event listener mobiliesiems
